...
server <- function(input, output) { 
  ...
  year2009_2010$InvoiceDate <- as.POSIXct(year2009_2010$InvoiceDate, format="%m/%d/%Y %H:%M")
  year2010_2011$InvoiceDate <- as.POSIXct(year2010_2011$InvoiceDate, format="%m/%d/%Y %H:%M")
  ...
  # entry_to_remove = c()
  # doubtful_entry = c()
  # data_check %>% filter(Quantity<0) -> cancellation
  # 
  # for(i in rev(cancellation$Index)){
  #   data_check %>% filter(`Customer.ID` == data_check[i,]$`Customer.ID`,
  #                         Description==data_check[i,]$Description,
  #                         StockCode == data_check[i,]$StockCode,
  #                         Quantity>=-1*data_check[i,]$Quantity) -> temp
  #   
  #   if(nrow(temp)==0){
  #     doubtful_entry = c(doubtful_entry, i)
  #   } else if(nrow(temp)>=1){
  #     data_check[temp[nrow(temp),]$Index,]$Cancelled_Q = -1*data_check[i,]$Quantity
  #     entry_to_remove = c(entry_to_remove, i)
  #     cancelled_list$Index = append(cancelled_list$Index, temp[nrow(temp),]$Index)
  #     cancelled_list$cancelled_Q = append(cancelled_list$cancelled_Q, data_check[i,]$Quantity)
  #   }
  #   cat(i)
  #   cat('\n')
  # }
  # 
  # write.csv(doubtful_entry, 'doubtful data.csv')
  # write.csv(entry_to_remove, 'entry to remove.csv')
  # write.csv(cancelled_list, 'index to add cancelled quantity.csv')
  ...
  length(doubtful_entry) #1935
  length(entry_to_remove) # 16809
  ...
  (entry_to_remove %>% length)/nrow(data_check)*100 # count for 2.03% of the dataframe entries
  ...
  sum(data_check$Cancelled_Q) # 420406

  dim(data_check) # 797885*10

  length(remove_data_vec)# 18744
  data_check %>% filter(Quantity<=0) %>% nrow # 18744
  data_check = data_check[-remove_data_vec,]
  dim(data_check) #805620*11
  data_check[,c(-1, -10)] %>% colnames # removing index cols
  data_check <- data_check[,c(-1, -10)]
  ...
  ###########################
  ###########################
  data_clean <- CustomerData # 805620*9
  ...
  colnames(data_clean)[ncol(data_clean)] <- "Spent"
